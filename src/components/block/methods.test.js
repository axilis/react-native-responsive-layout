...
describe('determineSize', () => {
  ...
  it('returns full width when nothing is defined', () => {
    expect(determineSize(sizeNames, 'sm', {})).toBeCloseTo(100);
  });

  it('returns expected size based on sizing class', () => {
    expect(determineSize(sizeNames, 'sm', { smSize: '1/2' })).toBeCloseTo(50);
  });

  it('returns full width when there is no class matching', () => {
    expect(determineSize(sizeNames, 'sm', { mdSize: '1/2' })).toBeCloseTo(100);
  });
  ...
  it('supports common size', () => {
    expect(determineSize(sizeNames, 'sm', { size: '1/3' })).toBeCloseTo(33.33);
  });

  it('supports overriding common size', () => {
    expect(determineSize(sizeNames, 'sm', { size: '1/3', smSize: '1/2' })).toBeCloseTo(50);
  });

  it('cascades from smaller to larger size', () => {
    expect(determineSize(sizeNames, 'md', { smSize: '1/2' })).toBeCloseTo(50);
  });

  it('respects overriding cascading sizes', () => {
    expect(determineSize(sizeNames, 'md', { smSize: '1/2', mdSize: '1/3' })).toBeCloseTo(33.33);
  });

  it('ignores overrides that do not apply', () => {
    expect(determineSize(sizeNames, 'md', { mdSize: '1/2', xlSize: '1/3' })).toBeCloseTo(50);
  });

  it('supports using auto as value', () => {
    expect(determineSize(sizeNames, 'md', { mdSize: 'auto' })).toEqual('auto');
  });
